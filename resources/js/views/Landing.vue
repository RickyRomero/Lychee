<template>
	<main v-if="loaded" id="landing" class="w-screen h-screen bg-black roboto overflow-hidden">
		<div id="header"
			...
			<div id="logo" class=" float-left p-4 text-text-main-0 translate-y-[-300px] opacity-0 animate-ladningAnimateDown">
				<a href="#">
					<h1 class="text-lg font-bold uppercase text-center roboto">{{ initdata.config.landing_title }}
			...
			<div id="menu" class="w-full translate-y-[-300px] opacity-0 animate-ladningAnimateDown">
				<ul class="menu list-none">
					<li class="menu-item relative block float-right pt-6 pb-5 px-3">
						<RouterLink to="/gallery" class="cursor-pointer block text-xs uppercase font-normal text-text-main-0 hover:text-text-main-400">{{ trans('lychee.GALLERY') }}</RouterLink>
					</li>
				</ul>
			</div>
		</div>

		<div id="intro" :class="{ hidden: !introVisible }"
			...
			<div id="intro_content" class=" self-center">
				<h1 class="
					text-center text-2xl  text-text-main-0 uppercase font-extralight animate-landingIntroPopIn">{{ initdata.config.landing_title }}</h1>
				<h2><span class="
					text-center text-base text-text-main-400 uppercase font-extralight animate-landingIntroPopIn">{{ initdata.config.landing_subtitle }}</span></h2>
			</div>
		</div>
